...
<ul>
	...
	<li>좀비 에너미 및 좀비 AI 구현</li>	
		<ul>
			...
			<li>총기류를 너무 쉽게 사용하지 못하게 하기위해 상당히 먼 거리의 총소리도 들을 수 있으며 소리의 발원지로 이동</li>
			<li>발원지로 이동하는 중 추가적인 소리가 감지되지 않으면 점점 이동속도가 줄어듦</li>
		</ul>	
	<li>데디케이트 서버 구현</li>	
		<ul>
			<li>작업분량 중 서버구현 관련이 상당량을 차지하게 될 걸 예상하였으나 여의치 않아 최대한 손 가지 않는 방식으로 구현
			<li>정석이라면 각 서버마다 세션을 열고, 클라이언트는 해당 세션을 찾아 들어가게 구성이 되어야 하겠지만 
			<li>세이프존 레벨과 필드 레벨을 고정포트로 설정하여 실행시키고 
			<li>클라이언트쪽에선 설정파일에 서버의 IP주소를 입력하여 url로 들어가는 방식으로 구현
			<li>맵에서 맵 이동또한 마찬가지로 설정된 고정포트의 url로 Travel진행
			<li>맵간 이동시 데이터는 소켓통신으로 넘기려 하였으나 세이브/로드파일로 구현이 가능하다 판단되어 드랍
		</ul>	
	<li>블랙보드[런처 to 런처 / 맵 to 맵] 구현</li>
	<li>세이브, 로드기능</li>
		<ul>
