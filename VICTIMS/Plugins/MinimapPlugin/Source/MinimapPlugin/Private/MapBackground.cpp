...
AMapBackground::AMapBackground()
{
	...
	// Don't capture every frame or on movement. We'll only render when we want to.
	CaptureComponent2D->bCaptureEveryFrame = false;
	CaptureComponent2D->bCaptureOnMovement = false;

	// Capture the scene with LDR color values
	CaptureComponent2D->CaptureSource = ESceneCaptureSource::SCS_FinalColorLDR;

	// Disable rendering features to render the world in a way that is convenient to draw over: auto-exposure, lighting, post-processing
...
#if WITH_EDITOR
	// Trigger USceneCaptureComponent2D logic to apply show flags. Normally this can be triggered through details customization only,
	// but this simulated event will apply it too.
...
void AMapBackground::PostEditMove(bool bFinished)
{
	...
	NormalizeScale();
	ApplyBackgroundTexture(/*bForceRerender=*/false);
	VisualizeLevelsInEditor();
}
#endif
...
{
	// Render a new background snapshot for each of the configured background levels
	ApplyBackgroundTexture(/*bForceRerender=*/true);
}
...
void AMapBackground::ApplyBackgroundTexture(bool bForceRerender)
{
	...
	for (int32 i = 0; i < BackgroundLevels.Num(); ++i)
	{
		...
		// Also trigger a re-render iff: background texture isn't set or if a rerender is forced
